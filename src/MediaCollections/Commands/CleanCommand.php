...
class CleanCommand extends Command
{
    ...
    protected function deleteOrphanedDirectories(): void
    {
        ...
        // Resolve the directories that are still in use through the configured
        // path generator(s) instead of assuming the default `{prefix}/{id}`
        // layout. This keeps orphan-directory cleanup working for custom path
        // generators (UUID-based, hash-based, ...).
        $usedDirectories = $this->mediaRepository->all()
            ->map(fn (Media $media) => $this->getTopLevelDirectory(PathGeneratorFactory::create($media)->getPath($media), $prefix))
            ->filter()
            ->collect()
            ->unique()
            ->flip();
        ...
        collect($directories)
            ...
            ->reject(fn (string $directory) => $usedDirectories->has($directory))
    ...
    /**
     * Get the top-level directory of a path generated by a path generator,
     * relative to the configured media-library prefix.
     */
